...
assert.equal(await page.locator('#bpm').inputValue(),'75');assert.equal(Number(await page.locator('#speed').inputValue()),4);assert.equal(await page.locator('#speed-number').inputValue(),'4.00');assert.equal(await page.locator('#speed').getAttribute('max'),'9.52');
...
await range('speed',9.52);await range('slope',35);await range('wingAngle',50);await page.screenshot({path:'output/settings/steep.png',fullPage:true});
