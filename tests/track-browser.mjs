...
await range('slope',10);await range('farDistance',58);await range('judgementZ',0);await page.locator('[data-tab="keys"]').click();await range('musicVolume',.25);await range('hitVolume',.75);await page.screenshot({path:'output/track/volumes.png',fullPage:true});await page.reload();await page.waitForFunction(()=>!!window.render_game_to_text);assert.equal((await state()).musicVolume,.25);assert.equal((await state()).hitVolume,.75);
await start();await page.waitForTimeout(2450);assert.ok(await page.evaluate(()=>window.ramps.some(v=>Math.abs(v-.012)<1e-6)));const nextNote=(await state()).notes[0];await jump(nextNote.time);await page.evaluate(()=>{window.ramps=[];});await page.keyboard.press(['ShiftLeft','a','s','d','f','Space'][nextNote.lane]);assert.ok(await page.evaluate(()=>window.ramps.some(v=>Math.abs(v-.036)<1e-6)));await back();
