...
Panel/audio refinement: settings sidebar has a fixed-height scrollable body and pinned full-reset/start footer; key/audio tab no longer has a duplicate reset. Music and hit sliders sit beneath their toggles, show 0% and disable when off, and recover saved levels when re-enabled. Both audio voices have twice the former 100% gain; metronome gain is unchanged. Judgement-line values now increase toward the far end (-4 near, +4 far), with fixed camera framing across line-position changes. Panel and track browser tests passed; final general regression and static build pending.
